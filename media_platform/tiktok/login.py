...
class TikTokLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        if config.LOGIN_TYPE == "qrcode":
            ...
            raise ValueError("[TikTokLogin.begin] Invalid Login Type Currently only supported qrcode or phone or cookie ...")
    ...
    async def popup_login_dialog(self):
        ...
        dialog_selector = "xpath=//div[contains(@class,'login') or contains(@id,'login')]"
        try:
            await self.context_page.wait_for_selector(dialog_selector, timeout=1000 * 10)
        except Exception as e:
            utils.logger.error(f"[TikTokLogin.popup_login_dialog] login dialog box does not pop up automatically, error: {e}")
            utils.logger.info("[TikTokLogin.popup_login_dialog] login dialog box does not pop up automatically, we will manually click the login button")
            login_button_ele = self.context_page.locator("xpath=//p[text() = '登录' or text() = 'Log in']")
            await login_button_ele.click()
            await asyncio.sleep(0.5)

    async def login_by_qrcode(self):
        utils.logger.info("[TikTokLogin.login_by_qrcode] Begin login tiktok by qrcode...")
        qrcode_img_selector = "xpath=//div[contains(@class,'qrcode')]//img"
        base64_qrcode_img = await utils.find_login_qrcode(
            self.context_page,
            selector=qrcode_img_selector
        )
        if not base64_qrcode_img:
            utils.logger.info("[TikTokLogin.login_by_qrcode] login qrcode not found please confirm ...")
            sys.exit()
    ...
    async def login_by_mobile(self):
        utils.logger.info("[TikTokLogin.login_by_mobile] Begin login tiktok by mobile ...")
        mobile_tap_ele = self.context_page.locator("xpath=//li[contains(text(), '验证码') or contains(text(), 'SMS')]")
        await mobile_tap_ele.click()
        await self.context_page.wait_for_selector("xpath=//input[contains(@placeholder,'手机号') or contains(@placeholder,'Phone')]")
        mobile_input_ele = self.context_page.locator("xpath=//input[contains(@placeholder,'手机号') or contains(@placeholder,'Phone')]")
        await mobile_input_ele.fill(self.login_phone)
        await asyncio.sleep(0.5)
        send_sms_code_btn = self.context_page.locator("xpath=//span[contains(text(), '获取验证码') or contains(text(), 'Send code')]")
        ...
        while max_get_sms_code_time > 0:
            utils.logger.info(f"[TikTokLogin.login_by_mobile] get tiktok sms code from redis remaining time {max_get_sms_code_time}s ...")
            await asyncio.sleep(1)
            sms_code_key = f"tt_{self.login_phone}"
            sms_code_value = cache_client.get(sms_code_key)
            if not sms_code_value:
                max_get_sms_code_time -= 1
                continue

            sms_code_input_ele = self.context_page.locator("xpath=//input[contains(@placeholder,'验证码') or contains(@placeholder,'code')]")
            await sms_code_input_ele.fill(value=sms_code_value.decode())
            await asyncio.sleep(0.5)
            submit_btn_ele = self.context_page.locator("xpath=//button[contains(@class,'login') or contains(@class,'submit')]")
    ...
    async def check_page_display_slider(self, move_step: int = 10, slider_level: str = "easy"):
        ...
        while not slider_verify_success:
            if max_slider_try_times <= 0:
                utils.logger.error("[TikTokLogin.check_page_display_slider] slider verify failed ...")
                sys.exit()
            try:
                ...
                if "操作过慢" in page_content or "提示重新操作" in page_content:
                    utils.logger.info("[TikTokLogin.check_page_display_slider] slider verify failed, retry ...")
                ...
                utils.logger.info("[TikTokLogin.check_page_display_slider] slider verify success ...")
                slider_verify_success = True
            except Exception as e:
                utils.logger.error(f"[TikTokLogin.check_page_display_slider] slider verify failed, error: {e}")
                await asyncio.sleep(1)
                max_slider_try_times -= 1
                utils.logger.info(f"[TikTokLogin.check_page_display_slider] remaining slider try times: {max_slider_try_times}")
                continue
